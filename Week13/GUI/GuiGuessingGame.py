...
def process(i):
    ...
    if guess == secretNumber:
        lbl = tk.Label(window, text="WINNER ", fg="green")
        ...
        if guess > secretNumber:
            lbl = tk.Label(window, text="Number is Lesser", fg="red")
            lbl.grid(row=guess_row, column=0, columnspan=5)
            lblLogs.append(lbl)
            guess_row += 1
        else:
            lbl = tk.Label(window, text="Number is Greater", fg="red")
            lbl.grid(row=guess_row, column=0, columnspan=5)
            lblLogs.append(lbl)
            guess_row += 1
    if totalNumberOfGuesses == 3:
        if guess != secretNumber:
            lbl = tk.Label(window, text="Max guesses reached", fg="red")
            lbl.grid(row=guess_row, column=0, columnspan=5)
            lblLogs.append(lbl)
            guess_row += 1
